...
import com.fimtra.infra.datafission.core.Context.NoopAtomicChangeManager;
...
final class Record implements IRecord, Cloneable
{
    ...
    static Record snapshot(IRecord template)
    {
        ...
        {
            ImmutableRecord clone = ((ImmutableRecord) template);
            return new Record(clone.name, clone.data, new NoopAtomicChangeManager(clone.contextName),
                newConcurrentHashMap(clone.subMaps)).clone();
        }
    }
    ...
    public boolean equals(Object o)
    {
        ...
        try
        {
            ...
            if (o instanceof ImmutableRecord)
            {
                ...
                return is.eq(this.name, other.getName()) && is.eq(this.context.getName(), other.getContextName())
