...
import com.fimtra.infra.datafission.core.RpcInstance.Remote;
import com.fimtra.infra.datafission.field.AbstractValue;
...
import com.fimtra.infra.util.CharBufferUtils;
            ...
            for (Iterator<Map.Entry<String, IValue>> it = entries.entrySet().iterator(); it.hasNext();)
            {
                ...
                else
                {
                    ...
                    switch(value.getType())
                    {
                        case DOUBLE:
                        case LONG:
                            // longs and doubles do not need escaping
                            txString.append(valueAsString);
                            break;
                        case BLOB:
                            // do these index of checks for binary data to prevent re-copying the
                            // array for a char-by-char scan
                            if ((valueAsString.indexOf(CR, 0) == -1) && (valueAsString.indexOf(LF, 0) == -1)
                                && (valueAsString.indexOf('\\', 0) == -1) && (valueAsString.indexOf('|', 0) == -1)
                                && (valueAsString.indexOf('=', 0) == -1))
                            {
                                txString.append(valueAsString);
                                break;
                            }
                            // fall back to a full char-by-char scan...
                            // todo this will hurt for a large binary as there is an array copy
                            //$FALL-THROUGH$
                        case TEXT:
                        default :
